...
export async function POST(req: NextRequest) {
  try {
    ...
    try {
      ...
      const customer = await asaas.createCustomer(customerPayload);
...
function getPlanPrice(planId: SubscriptionPlan): number {
  const prices = {
    ...
    [SubscriptionPlan.MICRO_BUSINESS]: 24.90,
    [SubscriptionPlan.SMALL_BUSINESS]: 49.90,
    [SubscriptionPlan.BUSINESS_SIMPLE]: 99.90,
    [SubscriptionPlan.BUSINESS_PLUS]: 149.90
  };
