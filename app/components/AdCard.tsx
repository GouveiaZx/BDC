...
// Cache para URLs limpas - evita processamento repetido
const urlCache = new Map<string, string>();

/**
 * Função utilitária memoizada para limpar URLs de imagens
...
const cleanImageUrl = (url: string): string => {
  ...
  // Cache do resultado (máximo 100 entradas)
  if (urlCache.size >= 100) {
    const firstKey = urlCache.keys().next().value;
    urlCache.delete(firstKey);
  }
  urlCache.set(url, cleanUrl);
 ...
 * @returns JSX.Element
 */
const AdCardComponent: React.FC<AdCardProps> = ({ ad, featured = false, onFavoriteToggle }) => {
  ...
  // Memoizar função de toggle favorito
  ...
  }, [isFavorite, onFavoriteToggle, ad.id]);
  
  // Memoizar handler do WhatsApp
  ...
  }, [ad.whatsapp, ad.title, openWhatsAppModal]);
  
  // Memoizar cálculo de rating
    ...
    <Link href={`/anuncio/${ad.id}`} className="block h-full">
      ...
      >
        ...
        <div className="relative h-48 md:h-56 w-full flex-shrink-0">
          ...
          {(() => {
            const totalImages = ad.photos?.length || ad.images?.length || 0;
        ...
        <div className="p-3 sm:p-4 flex-grow flex flex-col">
          ...
          <div className="flex items-start border-b border-gray-300 pb-2 mb-2">
            ...
            >
              <Avatar
                src={ad.userAvatar ? cleanImageUrl(ad.userAvatar) : undefined}
                alt={ad.userName || 'Avatar do vendedor'}
                size={40}
                fallbackName={ad.userName}
              />
...
const AdCard = memo(AdCardComponent, (prevProps, nextProps) => {
  // Comparação otimizada - só re-renderizar se props críticas mudaram
  return (
    prevProps.ad.id === nextProps.ad.id &&
    prevProps.ad.title === nextProps.ad.title &&
    prevProps.ad.price === nextProps.ad.price &&
    prevProps.ad.primary_photo === nextProps.ad.primary_photo &&
