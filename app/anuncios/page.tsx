...
export default function AnunciosPage() {
  ...
  // Adicionar estado para categorias dinâmicas
      ...
      try {
        ...
        if (response.ok) {
          ...
          if (data.success && data.categories) {
            // Adicionar opção "Todas as categorias" no topo
            const allOption = { id: 'todas', name: 'Todas as categorias' };
            const sorted = data.categories.sort((a: any, b: any) => a.name.localeCompare(b.name));
            setCategoriesList([allOption, ...sorted]);
          }
        }
      } catch (error) {
        console.error('Erro ao carregar categorias:', error);
        setCategoriesList([{ id: 'todas', name: 'Todas as categorias' }]);
      }
    };
    fetchCategories();
  }, []);
    ...
    const fetchAds = async () => {
      ...
      try {
        ...
        const formattedAds = ads.map(ad => ({
          ...
          views: ad.views || 0,
          clicks: ad.clicks || 0,
          userName: ad.userName || 'Anunciante', // ✅ Corrigido para usar userName da API
          userAvatar: ad.userAvatar || '/logo.png', // ✅ Corrigido para usar userAvatar da API
          userId: ad.user_id,
