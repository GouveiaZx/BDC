...
const StatusBadge = ({ status, moderationStatus }: { status: string, moderationStatus?: string }) => {
  // Usar moderationStatus se disponuu00e1vel, caso contru00e1rio usar status
  const displayStatus = moderationStatus || status;
    ...
    case 'expired':
      return (
        <div className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-gray-100 text-gray-800">
          <FaTimesCircle className="mr-1" /> Expirado
        </div>
      );
...
export default function MeusDestaques() {
  ...
  useEffect(() => {
    const fetchDestaques = async () => {
      try {
        ...
        const response = await fetch(`/api/destaques?userId=${userId}&status=all`, {
          headers,
        ...
        if (data.success && Array.isArray(data.destaques)) {
          ...
          const mappedDestaques = data.destaques.map((destaque: any) => ({
            ...
            // Usar o status real da API, não sobrescrever
            moderationStatus: destaque.status // Usar status direto da API
          }));
  ...
  const deleteDestaque = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(`Erro ao excluir destaque: ${response.status}`);
      }
      ...
      if (result.success) {
        ...
        throw new Error(result.error || 'Falha ao excluir destaque');
      }
    } catch (err) {
  ...
  // Estatu00edsticas dos destaques
  const activeDestaques = destaques.filter(d => d.status === 'approved' || d.moderationStatus === 'approved').length;
  const pendingDestaques = destaques.filter(d => d.status === 'pending' || d.moderationStatus === 'pending').length;
  const rejectedDestaques = destaques.filter(d => d.status === 'rejected' || d.moderationStatus === 'rejected').length;
