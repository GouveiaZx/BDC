...
export default function MeuPerfil() {
  ...
  const saveChanges = async () => {
    // Salvar alterações usando o hook de sincronização
    try {
      setIsLoading(true);
      ...
      // Upload do avatar se for diferente do padrão e do valor original
      if (avatarPreview && avatarPreview !== '/images/avatar-placeholder.png' && avatarPreview !== profile?.avatar) {
        try {
          const userId = profile?.id || localStorage.getItem('userId') || 'anonymous';
          ...
          if (uploadedAvatar) {
            ...
            // Salvar URL no localStorage para persistência imediata
            localStorage.setItem('userAvatarPreview', uploadedAvatar);
            console.log('Avatar salvo no localStorage:', uploadedAvatar);
          }
        } catch (error) {
      ...
      // Upload do banner se for diferente do padrão e do valor original
      if (bannerPreview && bannerPreview !== '/images/banner-placeholder.jpg' && bannerPreview !== profile?.banner) {
        try {
          ...
          if (uploadedBanner) {
            ...
            console.log('Banner salvo no localStorage:', uploadedBanner);
          }
        } catch (error) {
