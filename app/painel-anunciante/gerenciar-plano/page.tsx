...
import {
  ...
  FaArrowDown, FaPause, FaPlay, FaTrash, FaShieldAlt, FaClock
} from 'react-icons/fa';
...
const GerenciarPlanoPage = () => {
  ...
  const plansInfo = {
    [SubscriptionPlan.FREE]: {
      ...
      monthlyPrice: 'R$ 0,00',
      color: 'gray',
      features: ['1 anúncio a cada 60 dias', '30 dias de duração', 'Sem destaques', 'Destaques avulsos R$ 9,90']
    },
    [SubscriptionPlan.MICRO_BUSINESS]: {
      ...
      color: 'blue',
      features: ['2 anúncios simultâneos', '30 dias de duração', '1 destaque por dia', 'Destaques extras R$ 4,90']
    },
    [SubscriptionPlan.SMALL_BUSINESS]: {
      ...
      color: 'green',
      features: ['5 anúncios simultâneos', '30 dias de duração', '2 destaques por dia', 'Destaques extras R$ 4,90']
    },
    [SubscriptionPlan.BUSINESS_SIMPLE]: {
      ...
      color: 'purple',
      features: ['10 anúncios simultâneos', '30 dias de duração', '4 destaques por dia', 'Perfil de loja personalizado']
    },
    [SubscriptionPlan.BUSINESS_PLUS]: {
      ...
      color: 'yellow',
      features: ['20 anúncios simultâneos', '30 dias de duração', '8 destaques por dia', 'Suporte prioritário']
    }
  };
  ...
  const loadSubscriptionData = async (userId: string) => {
    try {
      ...
      // ✅ USAR NOVA API QUE SUPORTA TRIAL PERIOD
      ...
      if (response.ok) {
        ...
        if (result.success && result.subscription) {
          const sub = result.subscription;
            ...
            id: userId + '_subscription', // ID temporário
            ...
            trial_active: sub.trial_active || false,
            trial_days_remaining: sub.trial_days_remaining || 0,
            ...
            <div className="bg-white rounded-lg shadow-md p-6">
              ...
              <div className={`border-2 border-${currentPlanInfo.color}-200 bg-${currentPlanInfo.color}-50 rounded-lg p-4 mb-4`}>
                <div className="text-center">
                  ...
                  {/* ✅ EXIBIR INFORMAÇÕES DO TRIAL */}
                  {currentSubscription?.is_trial && currentSubscription?.trial_active && (
                    <div className="mt-3 p-3 bg-gradient-to-r from-green-100 to-blue-100 border border-green-200 rounded-lg">
                      <div className="flex items-center justify-center text-green-800 mb-1">
                        <FaClock className="mr-2" />
                        <span className="font-semibold">Período de Teste Gratuito</span>
                      </div>
                      <p className="text-sm text-green-700">
                        <strong>{currentSubscription.trial_days_remaining} dias restantes</strong>
                      </p>
                      <p className="text-xs text-green-600 mt-1">
                        Teste termina em: {currentSubscription.trial_ends_at ? new Date(currentSubscription.trial_ends_at).toLocaleDateString('pt-BR') : 'Data não definida'}
                      </p>
                    </div>
                  )}
                </div>
