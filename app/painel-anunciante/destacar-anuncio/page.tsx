...
import {
  ...
  FaClock, FaEye, FaInfoCircle, FaTags, FaPalette, FaGlobe, FaCrown
} from 'react-icons/fa';
import { toast } from 'react-hot-toast';
import { SubscriptionPlan } from '../../models/types';
import { getSubscriptionLimits, calculateFeaturePrice, canFeatureAd } from '../../config/subscription-limits';
...
const DestacarAnuncioPage = () => {
  ...
  // ✅ NOVO: Hook de assinatura para verificação de plano
  const { currentPlan, canCreateHighlight } = useSubscription();
  
  // ✅ NOVO: Estado para controle de validação
  ...
  // ✅ NOVO: Verificar limitações do plano
  useEffect(() => {
    if (currentPlan) {
      setUserPlan(currentPlan);
      
      // Verificar se o usuário pode criar destaques gratuitos
      const canCreateFreeHighlight = canFeatureAd(0, 0, 0, currentPlan);
      
      if (currentPlan === SubscriptionPlan.FREE) {
        setPlanValidation({
          canCreateFree: false,
          requiresPayment: true,
          message: 'Usuários do plano gratuito precisam pagar R$ 9,90 por destaque.'
        });
      } else {
        // Para planos pagos, verificar se ainda tem destaques disponíveis
        const limits = getSubscriptionLimits(currentPlan);
        setPlanValidation({
          canCreateFree: canCreateFreeHighlight,
          requiresPayment: !canCreateFreeHighlight,
          message: canCreateFreeHighlight 
            ? `Você ainda tem destaques disponíveis no seu plano ${limits.maxFeaturedPerDay} por dia.`
            : `Você atingiu o limite do seu plano. Destaques extras custam R$ ${limits.featuredPrice.toFixed(2)}.`
        });
      }
    }
  }, [currentPlan]);
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      ...
      {/* ✅ NOVO: Aviso sobre limitações do plano */}
      {planValidation.requiresPayment && (
        <div className={`mb-6 p-4 rounded-lg border ${
          ...
          <div className="flex items-start">
            <FaInfoCircle className={`mr-3 mt-0.5 ${
              currentPlan === SubscriptionPlan.FREE ? 'text-blue-600' : 'text-amber-600'
            }`} />
            <div>
              <h3 className={`font-semibold ${
                currentPlan === SubscriptionPlan.FREE ? 'text-blue-800' : 'text-amber-800'
              }`}>
                {currentPlan === SubscriptionPlan.FREE ? 'Destaque Pago' : 'Limite de Destaques'}
              </h3>
              <p className={`mt-1 ${
                currentPlan === SubscriptionPlan.FREE ? 'text-blue-700' : 'text-amber-700'
              }`}>
              ...
              {currentPlan === SubscriptionPlan.FREE && (
                <div className="mt-2">
                  ...
                  >
                    ...
                    Assinar um plano para destaques incluídos
                  </Link>
                </div>
              )}
            </div>
