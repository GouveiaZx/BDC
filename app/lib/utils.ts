...
export async function getDailyAdViews(days: number = 30, userId?: string): Promise<{ dailyViews: any[], error: any }> {
  try {
    ...
    if (error) {
      ...
      try {
        ...
        if (queryError) {
          ...
          // Criar dados fictícios para fallback em desenvolvimento
          if (typeof window !== 'undefined' && process.env.NODE_ENV === 'development') {
            const dailyViewsFallback = [];
            ...
            for (let i = 0; i < days; i++) {
              ...
              dailyViewsFallback.unshift({
                view_date: date.toISOString().split('T')[0],
                views: Math.floor(Math.random() * 10) // Dados fictícios para desenvolvimento
              });
            }
            
            console.log('Usando dados fictícios para visualizações diárias em modo de desenvolvimento');
            return { dailyViews: dailyViewsFallback, error: null };
          }
    ...
    // Fallback para dados fictícios em modo de desenvolvimento
    if (typeof window !== 'undefined' && process.env.NODE_ENV === 'development') {
      const dailyViewsFallback = [];
      const today = new Date();
      ...
      console.log('Usando dados fictícios para visualizações diárias em modo de desenvolvimento após erro geral');
      return { dailyViews: dailyViewsFallback, error: null };
    }
