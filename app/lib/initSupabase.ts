 ...
 * IMPORTANTE: Esta função só deve ser executada no servidor
 */
export async function initializeSupabaseTables(): Promise<{ success: boolean, error?: string }> {
  if (isBrowser) {
    console.warn('initializeSupabaseTables não deve ser executada no browser');
    return { success: false, error: 'Função apenas para servidor' };
  }
...
export async function createProfilesTableFunction(): Promise<{ success: boolean, error?: string }> {
  if (isBrowser) {
    console.warn('createProfilesTableFunction não deve ser executada no browser');
    return { success: false, error: 'Função apenas para servidor' };
  }
...
export async function createBusinessProfilesTableFunction(): Promise<{ success: boolean, error?: string }> {
  if (isBrowser) {
    console.warn('createBusinessProfilesTableFunction não deve ser executada no browser');
    return { success: false, error: 'Função apenas para servidor' };
  }
...
export async function createExecSQLFunction(): Promise<{ success: boolean, error?: string }> {
  if (isBrowser) {
    console.warn('createExecSQLFunction não deve ser executada no browser');
    return { success: false, error: 'Função apenas para servidor' };
  }
...
export async function initializeSupabaseServices() {
  if (isBrowser) {
    console.warn('initializeSupabaseServices não deve ser executada no browser');
    return { success: false, error: 'Função apenas para servidor' };
  }
 ...
 * Agora usa apenas o cliente público quando executada no browser
 */
export async function initializeSupabase() {
  try {
    console.log('Inicializando Supabase...');
    
    // Usar o cliente apropriado dependendo do ambiente
    const supabase = isBrowser ? getSupabaseClient() : supabaseAdmin;
    
    // Verificar se o usuário está autenticado
    const { data: { session } } = await supabase.auth.getSession();
    console.log('Status de autenticação:', session ? 'Usuário autenticado' : 'Sem autenticação');
    
    // Se o usuário estiver autenticado e estivermos no servidor, verificar tabelas
