...
const createTablesIfNeeded = async (client: SupabaseClient) => {
    try {
      // Verificar se as tabelas existem simplesmente tentando uma consulta
      const { data: profilesExists } = await client
        .from('profiles')
        .select('id')
        .limit(1)
        .maybeSingle();
...
const executeSchemaInitialization = async (client: SupabaseClient): Promise<boolean> => {
  try {
...
export function useProfileSync() {
  ...
  const SYNC_DEBOUNCE_MS = 300; // Debounce sync operations

  // Memoizar função para criar perfil base
  const createBaseProfile = useCallback((userId: string, userEmail: string | null): UserProfile => {
    const localData = getProfileDataFromLocalStorage();
    logger.debug('Criando perfil base com ID:', userId, 'e Email:', userEmail);
  ...
  // Memoizar função para inserir perfil no Supabase
  const insertProfileIntoSupabase = useCallback(async (
    adminClient: SupabaseClient, 
    profileToInsert: Partial<UserProfile>
  ): Promise<boolean | any> => {
    try {
      if (!profileToInsert.id) {
        logger.error('ID do perfil é obrigatório para inserção no Supabase');
        return false;
      }

      logger.debug('Tentando inserir/atualizar perfil no Supabase:', profileToInsert.id);
      ...
      if (profileToInsert.email) {
        ...
        if (emailError && emailError.code !== 'PGRST116') {
          logger.warn('Erro ao verificar email existente:', emailError);
        } else if (emailProfile) {
          existingProfile = emailProfile;
          logger.debug('✅ Perfil já existe com email:', emailProfile.email, 'ID:', emailProfile.id);
          
          if (emailProfile.id !== profileToInsert.id) {
            logger.debug('🔄 Perfil já existe com ID diferente, usando o existente');
      ...
      if (existingProfile) {
        logger.debug('🔄 Atualizando perfil existente');
        ...
        if (updateError) {
          logger.warn('Aviso ao atualizar perfil existente:', updateError);
          return existingProfile;
        } else {
          logger.debug('✅ Perfil existente atualizado com sucesso');
      ...
      if (profileError) {
        if (profileError.code === '23505' && profileError.details?.includes('email')) {
          logger.debug('⚠️ Email já existe, buscando perfil existente...');
          ...
          if (existingByEmail) {
            logger.debug('✅ Retornando perfil existente encontrado');
            localStorage.setItem('userId', existingByEmail.id);
            return existingByEmail;
          }
        }
        
        logger.warn('Erro ao fazer upsert na tabela profiles:', profileError);
        return false;
      }
      
      logger.debug('✅ Novo perfil criado com sucesso:', upsertedProfile);
      return upsertedProfile;
      
    } catch (error) {
      logger.warn('Erro ao inserir/atualizar perfil no Supabase:', error);
      return false;
    }
  }, []);

  // Função de load do profile memoizada
  const loadProfile = useCallback(async () => {
    // Debounce para evitar múltiplas chamadas
    const now = Date.now();
    if (now - lastSyncRef.current < SYNC_DEBOUNCE_MS) {
      logger.debug('Load profile debounced, aguardando...');
      return;
    }
    ...
    try {
      // Simplificar inicialização do schema
      if (!initializeSchemaPromise) {
        logger.debug('Iniciando inicialização do schema...');
        const supabaseClient = getSupabaseClient();
        initializeSchemaPromise = executeSchemaInitialization(supabaseClient);
      }
      await initializeSchemaPromise;
        ...
        try {
          const supabaseClient = getSupabaseClient();
          const { data: remoteProfileData, error: fetchError } = await supabaseClient
            .from('profiles')
            .select('*')
            .eq('id', userId)
            .maybeSingle();
          ...
          if (remoteProfileData) {
            const { data: businessRecord, error: businessError } = await supabaseClient
            ...
            const fetchedProfile = {
              ...
              avatar: remoteProfileData.avatar_url || localProfile?.avatar || '/images/avatar-placeholder.png',
            ...
            if (localProfile && localProfile.id) {
              ...
              await insertProfileIntoSupabase(supabaseClient, localProfile);
              setOfflineMode(false);
            } else {
              ...
              await insertProfileIntoSupabase(supabaseClient, baseProfile);
              setOfflineMode(false);
            }
          }
        } catch (syncError) {
  ...
  }, [createBaseProfile, insertProfileIntoSupabase]);
  ...
  // Função duplicada removida - usando a versão com useCallback acima
      ...
      try {
        ...
        console.warn('❌ Falha na API completa, usando método direto:', apiError);
         ...
         try {
           ...
           const supabaseClient = getSupabaseClient();
           const result = await insertProfileIntoSupabase(supabaseClient, profileData);
