...
async function checkManualAuth(request: NextRequest): Promise<boolean> {
  try {
    ...
    if (adminAuthCookie === 'true') {
      console.log('(Stats API) Autenticação manual válida via cookie admin-auth');
      return true;
    }
    
    // Se temos um token de acesso do Supabase, também consideramos autenticado
    if (sbAccessToken) {
      console.log('(Stats API) Autenticação manual válida via sb-access-token');
      return true;
    }
    
    return false;
  } catch (error) {
    console.error('(Stats API) Erro ao verificar autenticação manual:', error);
    return false;
  }
}
...
async function isAdmin(userId: string): Promise<boolean> {
  try {
    console.log('(Stats API) Verificando privilégios de admin para usuário:', userId);
    ...
    console.error('(Stats API) Erro ao verificar se usuário é admin:', error);
    return false;
  }
}
...
export async function GET(request: NextRequest) {
  try {
    ...
    if (sessionError || !session) {
      console.log('(Stats API) Sem sessão Supabase, verificando autenticação manual...');
      const hasManualAuth = await checkManualAuth(request);
      
      if (hasManualAuth) {
        console.log('(Stats API) Usando autorização manual para a API de estatísticas');
        isUserAdmin = true;
      } else {
        console.error('(Stats API) Sem sessão autenticada e sem autenticação manual');
      ...
      console.log('(Stats API) Sessão encontrada para usuário:', session.user.id);
      isUserAdmin = await isAdmin(session.user.id);
      
      if (!isUserAdmin) {
        console.warn('(Stats API) Acesso negado para usuário:', session.user.id, session.user.email);
        return NextResponse.json({ 
          success: false, 
    ...
    try {
      ...
      console.error('(Stats API) Erro ao contar total de assinaturas:', error);
    }
    ...
    try {
      ...
      console.error('(Stats API) Erro ao contar assinaturas por status:', error);
    }
    ...
    try {
      ...
      console.error('(Stats API) Erro ao calcular receita mensal:', error);
    }
    ...
    try {
      ...
      console.error('(Stats API) Erro ao contar tipos de conta:', error);
    }
    ...
    console.error('(Stats API) Erro ao processar requisição:', error);
    return NextResponse.json({ 
      success: false, 
