...
console.log('[Plans API] Configuração Supabase:');
console.log('[Plans API] URL:', supabaseUrl);
console.log('[Plans API] Service Key disponível:', !!supabaseServiceKey);
...
export async function GET(request: NextRequest) {
  try {
    console.log('[Plans API] Iniciando requisição GET');
    
    // Verificar autenticação
    if (!verifyAdminAuth(request)) {
      console.log('[Plans API] Falha na autenticação');
    ...
    console.log('[Plans API] Autenticação OK, buscando planos...');
    ...
    console.log('[Plans API] Teste de conexão - Count:', testData);
    console.log('[Plans API] Teste de conexão - Error:', testError);
    
    if (testError) {
      console.error('[Plans API] Erro no teste de conexão:', testError);
      return NextResponse.json({
        success: false,
    ...
    console.log('[Plans API] Query executada - Planos:', plans?.length || 0);
    console.log('[Plans API] Query executada - Error:', error);
    
    if (error) {
      console.error('[Plans API] Erro ao buscar planos:', error);
      return NextResponse.json({
        success: false,
    ...
    console.log('[Plans API] Encontrados', plans?.length || 0, 'planos');
    ...
    console.log('[Plans API] Planos formatados:', formattedPlans.length);
    ...
    console.error('[Plans API] Erro geral:', error);
    return NextResponse.json({ 
      success: false, 
