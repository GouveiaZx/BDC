...
export async function GET(request: NextRequest) {
  try {
    console.log('[Subscriptions API] Iniciando requisição GET simplificada');
    
    // Verificar autenticação
    if (!verifyAdminAuth(request)) {
      console.log('[Subscriptions API] Falha na autenticação');
    ...
    console.log('[Subscriptions API] Parâmetros:', { status, limit, offset });
    ...
    if (subsError) {
      console.error('[Subscriptions API] Erro ao buscar assinaturas:', subsError);
      return NextResponse.json({
        success: false,
    ...
    console.log('[Subscriptions API] Encontradas', subscriptions?.length || 0, 'assinaturas');
    ...
    if (uniqueUserIds.length > 0) {
      console.log('[Subscriptions API] Buscando dados para', uniqueUserIds.length, 'usuários únicos');
      ...
      try {
        ...
        if (!usersError && users) {
          ...
          console.log('[Subscriptions API] ✅ Tabela users encontrou', users.length, 'usuários');
        }
      } catch (error) {
        console.error('[Subscriptions API] Erro ao buscar na tabela users:', error);
      }
      ...
      if (missingIds.length > 0) {
        try {
          console.log('[Subscriptions API] Buscando', missingIds.length, 'usuários faltantes na tabela profiles');
          ...
          if (!profilesError && profiles) {
            ...
            console.log('[Subscriptions API] ✅ Tabela profiles encontrou', profiles.length, 'usuários');
          }
        } catch (error) {
          console.error('[Subscriptions API] Erro ao buscar na tabela profiles:', error);
        }
      }
      ...
      if (stillMissingIds.length > 0) {
        try {
          console.log('[Subscriptions API] Buscando', stillMissingIds.length, 'empresas em business_profiles');
          ...
          if (!businessError && businessProfiles) {
            ...
            console.log('[Subscriptions API] ✅ Business profiles encontrou', businessProfiles.length, 'empresas');
          }
        } catch (error) {
          console.error('[Subscriptions API] Erro ao buscar business_profiles:', error);
        }
      }
      ...
      if (finalMissingIds.length > 0) {
        try {
          console.log('[Subscriptions API] Buscando', finalMissingIds.length, 'empresas na tabela businesses');
          ...
          if (!businessError && businesses) {
            ...
            console.log('[Subscriptions API] ✅ Tabela businesses encontrou', businesses.length, 'empresas');
          }
        } catch (error) {
          console.error('[Subscriptions API] Erro ao buscar tabela businesses:', error);
        }
      }
      ...
      if (authMissingIds.length > 0) {
        try {
          console.log('[Subscriptions API] Buscando', authMissingIds.length, 'usuários em auth.users');
          ...
          for (const userId of authMissingIds.slice(0, 5)) { // Limitar para não sobrecarregar
            try {
              ...
              console.error(`[Subscriptions API] Erro ao buscar auth user ${userId}:`, singleUserError);
            }
          }
          
          const authFoundCount = authMissingIds.filter(id => usersMap[id]).length;
          if (authFoundCount > 0) {
            console.log('[Subscriptions API] ✅ Auth.users encontrou', authFoundCount, 'usuários');
          }
        } catch (error) {
          console.error('[Subscriptions API] Erro ao buscar auth.users:', error);
        }
      }
      
      console.log('[Subscriptions API] Total final de usuários mapeados:', Object.keys(usersMap).length, 'de', uniqueUserIds.length);
    }
    ...
    if (uniquePlanIds.length > 0) {
      try {
        ...
        console.error('[Subscriptions API] Erro ao buscar planos:', error);
      }
    }
    ...
    const formattedData = subscriptions.map(sub => {
      ...
      if (user.name && user.email && !userName.includes('não encontrado')) {
        console.log('[Subscriptions API] ✅ Usuário encontrado:', {
        ...
        console.log('[Subscriptions API] ⚠️ Usuário não encontrado para assinatura:', sub.id, 'user_id:', sub.user_id);
      }
    ...
    console.log('[Subscriptions API] Retornando', formattedData.length, 'registros formatados');
    ...
    console.error('[Subscriptions API] Erro geral:', error);
    return NextResponse.json({ 
      success: false, 
...
async function getBasicStats() {
  try {
    console.log('[Subscriptions API] Buscando estatísticas básicas...');
    ...
    if (error) {
      console.error('[Subscriptions API] Erro ao buscar estatísticas:', error);
    ...
    if (uniquePlanIds.length > 0) {
      try {
        ...
        console.error('[Subscriptions API] Erro ao buscar planos para estatísticas:', error);
      }
    }
    ...
    console.log('[Subscriptions API] Assinaturas pagas ativas:', paidActiveSubscriptions.length);
    console.log('[Subscriptions API] Assinaturas gratuitas ativas:', freeActiveSubscriptions.length);
    console.log('[Subscriptions API] Total de assinaturas pagas (todos status):', allPaidSubscriptions.length);
    ...
    console.log('[Subscriptions API] Estatísticas calculadas:', stats);
    
    return stats;
  } catch (error) {
    console.error('[Subscriptions API] Erro ao obter estatísticas:', error);
    return getDefaultStats();
  }
}
...
export async function PUT(request: NextRequest) {
  try {
    console.log('[Subscriptions API] Iniciando PUT');
    ...
    console.log('[Subscriptions API] Body recebido:', body);
    ...
    if (plan_id) {
      console.log('[Subscriptions API] Alterando plano para:', plan_id);
      ...
      console.log('[Subscriptions API] Plano validado:', newPlan.name);
    }
    ...
    console.log('[Subscriptions API] Dados para atualização:', updateData);
    ...
    if (error) {
      console.error('[Subscriptions API] Erro no update:', error);
      throw new Error(`Erro ao atualizar assinatura: ${error.message}`);
    }
    
    console.log('[Subscriptions API] Assinatura atualizada:', data);
    ...
    console.error('[Subscriptions API] Erro no PUT:', error);
    return NextResponse.json({ 
      success: false, 
      message: 'Erro interno do servidor',
      details: error instanceof Error ? error.message : 'Erro desconhecido'
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    console.error('[Subscriptions API] Erro no DELETE:', error);
    return NextResponse.json({ 
      success: false, 
      message: 'Erro interno do servidor' 
    }, { status: 500 });
