...
export async function GET(request: NextRequest) {
  try {
    console.log('[Reports API] Iniciando requisição GET');
    
    // Verificar autenticação
    if (!verifyAdminAuth(request)) {
      console.log('[Reports API] Falha na autenticação');
    ...
    console.log('[Reports API] Parâmetros:', { status, type, limit, offset });
    ...
    if (reportsError) {
      console.error('[Reports API] Erro ao buscar denúncias:', reportsError);
      return NextResponse.json({
        success: false,
    ...
    console.log('[Reports API] Encontradas', reports?.length || 0, 'denúncias');
    ...
    if (uniqueUserIds.length > 0) {
      try {
        ...
        console.error('[Reports API] Erro ao buscar usuários:', error);
      }
    }
    ...
    console.log('[Reports API] Retornando', formattedData.length, 'registros formatados');
    ...
    console.error('[Reports API] Erro geral:', error);
    return NextResponse.json({ 
      success: false, 
...
async function getReportsStats() {
  try {
    console.log('[Reports API] Buscando estatísticas...');
    ...
    if (error) {
      console.error('[Reports API] Erro ao buscar estatísticas:', error);
    ...
    console.log('[Reports API] Estatísticas calculadas:', stats);
    
    return stats;
  } catch (error) {
    console.error('[Reports API] Erro ao obter estatísticas:', error);
    return getDefaultReportsStats();
  }
}
...
export async function PUT(request: NextRequest) {
  try {
    console.log('[Reports API] Iniciando PUT');
    ...
    console.log('[Reports API] Body recebido:', body);
    ...
    console.log('[Reports API] Dados para atualização:', updateData);
    console.log('[Reports API] ID da denúncia:', id);
    ...
    if (error) {
      console.error('[Reports API] Erro no update:', error);
      throw new Error(`Erro ao atualizar denúncia: ${error.message}`);
    }
    
    console.log('[Reports API] Denúncia atualizada:', data);
    ...
    console.error('[Reports API] Erro no PUT:', error);
    return NextResponse.json({ 
      success: false, 
      message: 'Erro interno do servidor',
      details: error instanceof Error ? error.message : 'Erro desconhecido'
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    console.error('[Reports API] Erro no DELETE:', error);
    return NextResponse.json({ 
      success: false, 
      message: 'Erro interno do servidor'
    }, { status: 500 });
