...
export default function AnuncioDetalhes() {
  ...
  useEffect(() => {
    const fetchAdDetails = async () => {
      try {
        ...
        // Dados do usuário vêm diretamente da RPC (já priorizados: users > profiles > fallback)
  ...
  // Obter array de imagens usando a nova estrutura
  const getImageArray = () => {
    if (ad.photos && ad.photos.length > 0) {
      return ad.photos.sort((a, b) => a.sort_order - b.sort_order).map(photo => photo.file_url);
    }
    return ad.images || [];
  };
  ...
  const getCleanSelectedImage = () => {
    ...
    if (!imageArray[selectedImage]) return '/images/placeholder.png';
    return cleanImageUrl(imageArray[selectedImage]);
  };
