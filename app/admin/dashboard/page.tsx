...
export default function AdminDashboard() {
  ...
  const [precosPlanosReais, setPrecosPlanosReais] = useState<{
    basico: number;
    premium: number;
    pro: number;
  }>({ basico: 29.90, premium: 59.90, pro: 99.90 }); // valores padrão como fallback
  ...
  const fetchDashboardData = async () => {
    try {
      ...
      // 6. Buscar preços reais dos planos
      try {
        const planosResponse = await fetch('/api/admin/plans?limit=10');
        if (planosResponse.ok) {
          const planosResult = await planosResponse.json();
          const planos = planosResult.data || planosResult.plans || [];
          
          console.log('📋 Planos encontrados:', planos.length);
          
          const precosReais = { basico: 29.90, premium: 59.90, pro: 99.90 }; // fallback
          
          planos.forEach((plano: any) => {
            const slug = plano.slug || plano.name?.toLowerCase();
            const preco = parseFloat(plano.price_monthly || plano.monthly_price || 0);
            
            if (preco > 0) {
              if (slug?.includes('basic') || slug?.includes('basico')) {
                precosReais.basico = preco;
              } else if (slug?.includes('premium')) {
                precosReais.premium = preco;
              } else if (slug?.includes('pro')) {
                precosReais.pro = preco;
              }
            }
          });
          
          setPrecosPlanosReais(precosReais);
          console.log('💰 Preços reais dos planos:', precosReais);
        }
      } catch (error) {
        console.warn('Não foi possível buscar preços dos planos:', error);
      }
      ...
      setStats(novasStats);
      await gerarDadosGrafico(assinaturas);
      await gerarAtividadesRecentes(assinaturas, empresas);
      ...
      await gerarDadosGrafico([]);
      await gerarAtividadesRecentes([], []);
    } finally {
      setLoading(false);
    }
  };
  
  // Função para gerar dados históricos reais do gráfico
  const gerarDadosGrafico = async (assinaturas: any[]) => {
    try {
      console.log('🔄 Gerando dados históricos reais do gráfico...');
      ...
      // Buscar dados históricos reais das assinaturas
      try {
        const historicoResponse = await fetch('/api/admin/subscriptions?limit=1000&status=all');
        let todasAssinaturas = assinaturas; // fallback para as assinaturas já carregadas
        
        if (historicoResponse.ok) {
          const historicoData = await historicoResponse.json();
          todasAssinaturas = historicoData.data || assinaturas;
          console.log('📊 Carregadas', todasAssinaturas.length, 'assinaturas para análise histórica');
        }
        
        // Processar dados dos últimos 6 meses
        for (let i = 5; i >= 0; i--) {
          const indiceMes = (mesAtual - i + 12) % 12;
          const anoDoMes = mesAtual - i < 0 ? anoAtual - 1 : anoAtual;
          
          // Filtrar assinaturas criadas neste mês
          const assinaturasDoMes = todasAssinaturas.filter((sub: any) => {
            const dataCreated = sub.createdAt || sub.created_at;
            if (!dataCreated) return false;
            
            const dataCriacao = new Date(dataCreated);
            return dataCriacao.getMonth() === indiceMes && dataCriacao.getFullYear() === anoDoMes;
          });
          
          // Contar por tipo de plano
          const basico = assinaturasDoMes.filter((sub: any) => {
            const planType = sub.planType || sub.plan_type;
            return planType === 'basic' || planType === 'basico';
          }).length;
          
          const premium = assinaturasDoMes.filter((sub: any) => {
            const planType = sub.planType || sub.plan_type;
            return planType === 'premium';
          }).length;
          
          const pro = assinaturasDoMes.filter((sub: any) => {
            const planType = sub.planType || sub.plan_type;
            return planType === 'pro';
          }).length;
          
          const total = basico + premium + pro;
          ...
          console.log(`📈 ${meses[indiceMes]}/${anoDoMes}: ${total} assinaturas (${basico} básico, ${premium} premium, ${pro} pro)`);
        }
        
      } catch (error) {
        console.error('❌ Erro ao buscar dados históricos, usando apenas dados atuais:', error);
        
        // Fallback: usar apenas dados reais atuais (sem simulação)
        const porPlanoAtual = {
          basico: assinaturas.filter(sub => (sub.planType || sub.plan_type) === 'basic').length,
          premium: assinaturas.filter(sub => (sub.planType || sub.plan_type) === 'premium').length,
          pro: assinaturas.filter(sub => (sub.planType || sub.plan_type) === 'pro').length
        };
        
        // Mostrar apenas o mês atual com dados reais
        const indiceMesAtual = mesAtual;
        dadosGrafico.push({
          mes: meses[indiceMesAtual],
          basico: porPlanoAtual.basico,
          premium: porPlanoAtual.premium,
          pro: porPlanoAtual.pro,
          total: porPlanoAtual.basico + porPlanoAtual.premium + porPlanoAtual.pro
        });
        
        // Para os outros meses, mostrar zero (dados históricos não disponíveis)
        for (let i = 5; i >= 1; i--) {
          const indiceMes = (mesAtual - i + 12) % 12;
        ...
        console.log('⚠️ Usando apenas dados atuais - histórico não disponível');
      }
      
      console.log('✅ Dados do gráfico gerados:', dadosGrafico);
      setChartData(dadosGrafico);
      
    } catch (error) {
      console.error('❌ Erro crítico ao gerar dados do gráfico:', error);
      
      // Fallback de emergência - dados vazios
      const meses = ['Jan', 'Fev', 'Mar', 'Abr', 'Mai', 'Jun', 'Jul', 'Ago', 'Set', 'Out', 'Nov', 'Dez'];
      const mesAtual = new Date().getMonth();
      const dadosVazios: ChartData[] = [];
      ...
      setChartData(dadosVazios);
    }
  };

  // Função para gerar atividades recentes com dados reais
  const gerarAtividadesRecentes = async (assinaturas: any[], empresas: any[]) => {
    try {
      console.log('🔄 Gerando atividades recentes com dados reais...');
      const atividades: AtividadeRecente[] = [];
      
      // 1. Adicionar assinaturas recentes (máximo 2)
      assinaturas.slice(0, 2).forEach((sub) => {
        atividades.push({
          id: `sub-${sub.id}`,
          tipo: 'assinatura',
          titulo: `Nova assinatura ${sub.planType || sub.plan_type || 'Básica'}`,
          descricao: `${sub.subscriber?.name || sub.user?.name || 'Usuário'} assinou o plano`,
          data: sub.createdAt || sub.created_at || new Date().toISOString(),
          status: sub.status === 'active' ? 'ativo' : 'pendente'
        });
      });
      
      // 2. Adicionar empresas recentes (máximo 1)
      empresas.slice(0, 1).forEach((emp) => {
        atividades.push({
          id: `emp-${emp.id}`,
          tipo: 'empresa',
          titulo: `Nova empresa: ${emp.businessName || emp.company_name}`,
          descricao: `Empresa ${emp.verified ? 'verificada' : 'aguardando verificação'}`,
          data: emp.createdAt || emp.created_at || new Date().toISOString(),
          status: emp.verified ? 'ativo' : 'pendente'
        });
      });
      
      // 3. Buscar anúncios recentes
      try {
        const anunciosResponse = await fetch('/api/admin/ads?limit=10&status=all');
        if (anunciosResponse.ok) {
          const anunciosData = await anunciosResponse.json();
          const anuncios = anunciosData.data || [];
          
          // Adicionar anúncios recentes (máximo 2)
          anuncios.slice(0, 2).forEach((ad: any) => {
            let statusAnuncio: "novo" | "ativo" | "pendente" | "cancelado" = 'pendente';
            if (ad.status === 'approved' || ad.moderation_status === 'approved') {
              statusAnuncio = 'ativo';
            } else if (ad.status === 'rejected' || ad.moderation_status === 'rejected') {
              statusAnuncio = 'cancelado';
            }
            
            atividades.push({
              id: `ad-${ad.id}`,
              tipo: 'anuncio',
              titulo: `Novo anúncio: ${ad.title?.slice(0, 30) || 'Anúncio'}${ad.title?.length > 30 ? '...' : ''}`,
              descricao: `${ad.user?.name || ad.profiles?.name || 'Usuário'} publicou um anúncio`,
              data: ad.created_at || new Date().toISOString(),
              status: statusAnuncio
            });
          });
          
          console.log('✅ Adicionados', Math.min(2, anuncios.length), 'anúncios às atividades');
        }
      } catch (error) {
        console.warn('⚠️ Não foi possível buscar anúncios para atividades:', error);
      }
      
      // 4. Buscar highlights/destaques recentes
      try {
        const highlightsResponse = await fetch('/api/admin/highlights?limit=5');
        if (highlightsResponse.ok) {
          const highlightsData = await highlightsResponse.json();
          const highlights = highlightsData.data || [];
          
          // Adicionar highlights recentes (máximo 1)
          highlights.slice(0, 1).forEach((highlight: any) => {
            let statusDestaque: "novo" | "ativo" | "pendente" | "cancelado" = 'ativo';
            if (!highlight.isActive) {
              statusDestaque = 'cancelado';
            } else if (new Date(highlight.expiresAt) < new Date()) {
              statusDestaque = 'cancelado';
            }
            
            atividades.push({
              id: `highlight-${highlight.id}`,
              tipo: 'anuncio', // Usar ícone de anúncio para highlights
              titulo: `Novo destaque: ${highlight.title?.slice(0, 30) || 'Destaque'}${highlight.title?.length > 30 ? '...' : ''}`,
              descricao: `${highlight.user?.name || 'Usuário'} publicou um destaque`,
              data: highlight.createdAt || new Date().toISOString(),
              status: statusDestaque
            });
          });
          
          console.log('✅ Adicionado', Math.min(1, highlights.length), 'destaque às atividades');
        }
      } catch (error) {
        console.warn('⚠️ Não foi possível buscar highlights para atividades:', error);
      }
      ...
      const atividadesFinais = atividades.slice(0, 5);
      
      console.log('✅ Atividades recentes geradas:', atividadesFinais.length, 'atividades');
      setAtividadesRecentes(atividadesFinais);
      
    } catch (error) {
      console.error('❌ Erro ao gerar atividades recentes:', error);
      
      // Fallback: usar apenas assinaturas e empresas
      const atividades: AtividadeRecente[] = [];
      
      assinaturas.slice(0, 3).forEach((sub) => {
        atividades.push({
          id: `sub-${sub.id}`,
          tipo: 'assinatura',
          titulo: `Nova assinatura ${sub.planType || 'Básica'}`,
          descricao: `${sub.subscriber?.name || 'Usuário'} assinou o plano`,
          data: sub.createdAt || sub.created_at || new Date().toISOString(),
          status: sub.status === 'active' ? 'ativo' : 'pendente'
        });
      });
      
      empresas.slice(0, 2).forEach((emp) => {
        atividades.push({
          id: `emp-${emp.id}`,
          tipo: 'empresa',
        ...
        <div className="p-6 border-t border-gray-200">
          <div className="grid grid-cols-3 gap-4">
            <div className="text-center">
              ...
              <p className="text-xs text-gray-500">{formatCurrency(precosPlanosReais.basico)}/mês</p>
                    </div>
            <div className="text-center">
              <div className="flex items-center justify-center">
                <div className="w-3 h-3 bg-purple-500 rounded-full mr-2"></div>
              ...
              <p className="text-xs text-gray-500">{formatCurrency(precosPlanosReais.premium)}/mês</p>
                  </div>
            <div className="text-center">
              <div className="flex items-center justify-center">
                <div className="w-3 h-3 bg-blue-600 rounded-full mr-2"></div>
              ...
              <p className="text-xs text-gray-500">{formatCurrency(precosPlanosReais.pro)}/mês</p>
                  </div>
                </div>
              </div>
            </div>
